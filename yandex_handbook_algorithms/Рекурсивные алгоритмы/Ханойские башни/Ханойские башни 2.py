...
Попробуем придумать алгоритм, который гарантировано переложит из этой конфигурации
...
Итого получаем рекурентное соотношение: g(n) = 2*g(n-2) + 3

Докажем, что g(n) = f(n):

В любом алгритме (который гарантировано решит задачу) будет момент, когда 
самый большой диск и следуюший по порядку нужно будет переложить на пустой стержень 4 (первое взаимодействие с 4ртым стержнем),
